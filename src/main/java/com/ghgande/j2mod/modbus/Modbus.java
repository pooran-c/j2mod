...
public interface Modbus {
	...
	/**
	 * Defines the class 0 function code for <tt>read multiple registers</tt>. The
	 ...
	 *
	 * Write some number of registers, then read some number of potentially other
	 * registers back.
	 */
...
//	int READ_MEI = 43;
	...
	/**
	 * Defines the maximum number of bits in multiple read/write of input discretes
	 * or coils (<b>2000</b>).
	 */
	int MAX_BITS = 2000;

	/**
	 * Defines the Modbus slave exception offset that is added to the function code,
	 * to flag an exception.
	 */
	...
	/**
	 * Defines the Modbus slave exception type <tt>illegal data address</tt>. This
	 * exception code is returned if the reference:
	 * <ul>
	...
	/**
	 * Defines the Modbus slave exception type <tt>slave device failure</tt>. This
	 * exception code indicates a fault in the slave device itself.
	 */
	int SLAVE_DEVICE_FAILURE = 4;

	/**
	 * Defines the Modbus slave exception type <tt>slave busy</tt>. This exception
	 * indicates the the slave is unable to perform the operation because it is
	 * performing an operation which cannot be interrupted.
	 */
	 ...
	 * respond</tt>. This exception code indicates that a Modbus gateway failed to
	 * receive a response from the specified target.
	 */
	 ...
	 * automatically based on the baud rate. Setting this value to anything other
	 * than zero will bypass that process and force a specific inter-message delay
	 * (=<tt>0</tt>).
	 */
	...
	/**
	 * The number of characters delay that is the allowed maximum between characters
	 * on the same serial port (within the same transaction)
	 */
	 ...
	 *
	 * <p>
	 * <b>Note:</b> The standard requires that the server copy whatever value the
	 * client provides. However, the transaction ID is being limited to signed
	 * 16-bit integers to prevent problems with servers that might incorrectly
